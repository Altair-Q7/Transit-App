...
class _TripControlScreenState extends State<TripControlScreen> {
  ...
  Future<void> _startTrip() async {
    ...
    try {
      final tripId = await _api.startTrip(busId: 1, routeId: 1);
      setState(() {
        _tripId = tripId;
        _tripActive = true;
      });
  ...
  Future<void> _reportIncident(String type) async {
    if (_tripId == null) return;
    try {
      await _api.reportIncident(tripId: _tripId!, type: type);
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              children: [
                Expanded(child: _IncidentTile(label: 'Breakdown', onTap: () => _reportIncident('breakdown'))),
                const SizedBox(width: 8),
                Expanded(child: _IncidentTile(label: 'Traffic', onTap: () => _reportIncident('traffic'))),
                const SizedBox(width: 8),
                Expanded(
                  child: _IncidentTile(label: 'Emergency', onTap: () => _reportIncident('emergency'), flagged: true),
                ),
              ],
            ),
          ],
